    ...
    <div className="space-y-6">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-foreground">
          Max price: {maxPrice.toLocaleString()} ISK
        </label>
        <input
          type="range"
          min={3000}
      ...
      <div className="container py-8">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">{filtered.length} tutors found</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setShowFilters(!showFilters)}
              className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-steel md:hidden"
            >
        ...
        <div className="mt-6 flex gap-8">
          ...
          {showFilters && (
            <div className="fixed inset-0 z-50 bg-foreground/50 md:hidden" onClick={() => setShowFilters(false)}>
              <div
                className="absolute bottom-0 left-0 right-0 max-h-[80vh] overflow-y-auto rounded-t-2xl bg-background p-6"
          ...
          <div className="flex-1">
            {filtered.length === 0 ? (
              <div className="rounded-lg border border-border bg-card py-16 text-center">
                ...
                <p className="mt-4 text-lg font-medium text-foreground">No tutors found</p>
